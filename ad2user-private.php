...
//  /usr/local/sbin/ad2user-private.php
//  construct NIS user with GID=UID map from group containing users (use CN of group containing Unix users)
...
// args[7]  OU containing private group definitions ( " )
    ...
    $sidinhex = str_split(bin2hex($object[0]['objectsid'][0]), 2);

    // Byte 8 count of sub authorities - Get number of sub-authorities
    $subauths = hexdec($sidinhex[7]);

    $start = 8 + (4 * ($subauths-1));
    $grid = hexdec($sidinhex[$start+3].$sidinhex[$start+2].$sidinhex[$start+1].$sidinhex[$start]);
    ...
    for ($i=0; $i<$account['count']; $i++)
    {
        ...
        echo $account[$i]['displayname'][0].":/home/$user:/bin/bash\n";
